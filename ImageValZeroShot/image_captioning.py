...
Supports GPU, optional quantization (4-bit), or CPU fallback if needed.
"""
...
class ArabicImageCaptioner:
    def __init__(self, model_name="Qwen/Qwen2.5-VL-7B-Instruct",
                 checkpoint_path=None, use_cpu=False, use_quant=False):
    ...
    def load_model(self):
        ...
        if self.use_cpu:
            ...
            model_args["device_map"] = "auto"
            model_args["torch_dtype"] = torch.float16  # standard FP16 for GPU
        ...
        # If the processor supports image resizing
        if hasattr(self.processor, "image_processor"):
            self.processor.image_processor.size = {"height": 512, "width": 512}

        print("✅ Model & processor loaded.")
        print(f"Device map: {getattr(self.model, 'hf_device_map', 'Unknown')}")
    ...
    def process_folder(self, image_folder, output_csv, supported_formats=('.png', '.jpg', '.jpeg')):
        ...
        if not image_files:
            print(f"⚠️ No supported images in {image_folder}")
            return
        ...
        with open(output_csv, mode='w', newline='', encoding='utf-8') as file:
            ...
            for image_file in tqdm(image_files, desc="Processing"):
                ...
                if caption:
                    ...
                    print(f"❌ Failed for {image_file}")

        print(f"✅ Done! Captions saved to: {output_csv}")


def main():
    parser = argparse.ArgumentParser(description="Arabic Image Captioning")
    ...
    parser.add_argument("--use_quant", action="store_true", help="Use 4-bit quantization")
